...
router.get('/fetchUpcomingAppointments/:doctorID', async(req, res) => {
    try {
        ...
        const sql='select appointment_id,patient_id,p.full_name as patient_name,appointment_date,time_slot,appointment_mode,appointment_status,appointment_type from appointments join patients p using(patient_id) where doctor_id=:1 and appointment_status=:2';
...
router.get('/allAppointments/:doctorID',async(req,res)=>{
    try {
        ...
        const sql="select appointment_id,patient_id,p.full_name as patient_name,appointment_date,time_slot,appointment_mode,appointment_status,appointment_type from appointments join patients p using (patient_id) where doctor_id=:1";
        const result=await con.execute(sql,binds);
        console.log(result);
        if(result.rows.length===0){
